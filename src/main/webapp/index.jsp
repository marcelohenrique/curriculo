<html>
	<header>
		<title>Marcelo Henrique - Currículo</title>
	</header>
	<body>

		<h2>Marcelo Henrique de Oliveira Lima</h2>

		Casado, brasileiro, 28 anos<br>
		SQN 312 Bloco D, 108 - Asa Norte<br>
		CEP: 70765-040 - Brasília - DF<br>
		Celular: (61) 8202-0300 / Telefone: (61) 3033-4822<br>
		E-mail: marcelohenriquedeoliveiralima@gmail.com<br>

		<h5>Resumo profissional</h5>

		Possuo oito anos de experiência com desenvolvimento, análise e elaboração de arquitetura envolvendo a plataforma Java e com processos como RUP, MPS.BR e Scrum. Atuo como líder técnico e arquiteto nos projetos que participo. Além de ter experiência como coordenador de equipes de projetos Java EE.

		<h5>Experiência profissional</h5>

		<ul>
			<li>
				Ministério Público do Distrito Federal e Territórios - 2014/Atual<br>
				Cargo: Técnico de Tecnologia da Informação e Comunicação.<br>
				Trabalho totalmente voltado para desenvolvimento de aplicações utilizando tecnologia Java web.
			</li>
			<li>
				Fóton Informática - 2013/2014<br>
				Cargo: Arquiteto de Software.<br>
				Processo de trabalho totalmente aderente ao RUP. Desenvolvimento da arquitetura de referência da empresa. Análise e projeto de sistemas/produtos. Desenvolvimento de componentes de software visando reuso e redução de trabalho.
			</li>
			<li>
				Ebix Latin America - 2010/2013<br>
				Cargo: Analista Líder.<br>
				Líder técnico e coordenador de equipes de projetos. Análise, projeto e implementação de aplicações web sobre a plataforma Java. Definição de arquitetura de aplicações web de alta complexidade na área de seguros.
			</li>
			<li>
				Usix Technology - 2008/2010<br>
				Cargo: Desenvolvedor.<br>
				Integrante da divisão de arquitetura da empresa cujo objetivo era gerar laudos, documentação, realizar auditoria nas aplicações, difundir novas técnicas e elevar o nível técnico das equipes de desenvolvimento.
			</li>
			<li>
				Sena Informática - 2007/2008<br>
				Cargo: Desenvolvedor.<br>
				Desenvolvimento de aplicações e soluções para sistemas Java utilizando tecnologia Web para a área de seguros.
			</li>
		</ul>

		<h5>Últimos Projetos</h5>

		<ul>
			<li>
				Grifo<br>
				Papel: Desenvolvedor.<br>
				Tecnologias: WebWork + Velocity, Spring, Hibernate, Tomcat 6.
			</li>
			<li>
				Ouvidoria Internet<br>
				Papel: Desenvolvedor.<br>
				Tecnologias: BootStrap, REST (RestEasy), JSON, Spring, Hibernate, JBoss.
			</li>
			<li>
				Certidão de Envolvimento em Feitos<br>
				Papel: Desenvolvedor.<br>
				Tecnologias: Servlet, JSP, Hibernate, REST (RestEasy), JSON.
			</li>
			<li>
				BRB-MyBank<br>
				Papel: Arquiteto de Software.<br>
				Tecnologias: JSF 2.0, Primefaces 3.5, EJB 3.1, JPA 2.0 (Hibernate), CDI 1.0, JAX-WS, JAX-B, JBoss AS 7.1 (desenvolvimento) e Weblogic 3.1.2 (homologação e produção).
			</li>
			<li>
				Worksite<br>
				Papel: Analista de sistemas. Responsável pela refatoração visando melhorias de performance da aplicação.<br>
				Tecnologias: JSF 1.2, Richfaces 3, Tomahawk, EJB 3, JPA (Hibernate), JAX-WS, JAX-B, Glassfish (desenvolvimento) e Websphere (homologação e produção).
			</li>
			<li>
				WIS Auto Unificados<br>
				Papel: Coordenador e líder técnico. Responsável pela implantação do projeto junto ao cliente (Sulamérica Seguros - RJ)<br>
				Tecnologias: JQuery, DWR 3, JavaBean-Validation, JAX-B, JAX-WS, Spring-JDBC, Maven, Tomcat (desenvolvimento) e Websphere (homologação e produção).
			</li>
			<li>
				Assistência 24 Horas<br>
				Papel: Arquiteto de Software.<br>
				Tecnologias: JSF 1.2, Facelets, Richfaces 3, Tomahawk, Spring, JPA (Hibernate), JAX-WS, Maven, Glassfish (desenvolvimento) e Websphere (homologação e produção).
			</li>
			<li>
				WIS Meta-Modelo<br>
				Papel: Arquiteto e Analista de sistemas (análise e projeto utilizando UML 2.0).<br>
				Tecnologias: Java, JavaScript, JQuery, PL/SQL, Tomcat.
			</li>
			<li>
				ESPA<br>
				Papel: Arquiteto.<br>
				Tecnologias: Struts 1.1, DWR 2, EJB 2.1 e JDBC.
			</li>
			<li>
				WIS Multicálculo Honda<br>
				Papel: Programador e líder técnico. Responsável pela implantação do projeto junto ao cliente (Honda Seguros - SP)<br>
				Tecnologias: DWR 2, Axis 2, JDBC, Castor e Quartz.
			</li>
		</ul>

		<h5>Formação acadêmica</h5>

		2004/2008 - Bacharel em Ciência da Computação pela Universidade Estadual do Ceará.

		<h5>Certificações</h5>

		2014 - Certified ScrumMaster.<br>
		2013 - Oracle Certified Professional, Java EE 5 Web Component Developer.<br>
		2012 - Programming in HTML5 with JavaScript and CSS3.<br>
		2010 - Sun Certified Programmer for the Java Platform, SE 6.<br>
		2009 - Sun Certified Associate for Java Platform, SE.

		<h5>Cursos</h5>
		
		Coursera - University of Maryland - Programming Mobile Applications for Android Handheld Systems.<br>
		Coursera - École Polytechnique Fédérale de Lausanne - Principles of Reactive Programming.

		<h5>Idiomas</h5>

		Inglês - Leitura: ótima; escrita e conversação: boa.

		<h5>Habilidades técnicas</h5>

		<ul>
			<li>
				Java
				<ul>
					<li>Servlet&JSP, EJB 2.x e 3.x, JasperReports + iReport, DWR, Quartz.</li>
					<li>Frameworks web: Struts 1 e 2, WebWork, JSF 1.x e 2.x, Facelets, Richfaces, Icefaces, Tomahawk, Primefaces.</li>
					<li>XML: Castor, XStream, JAX-B.</li>
					<li>Web Services: Axis 1 e 2, XFire, JAX-WS.</li>
					<li>Teste: JUnit, Arquillian, DBUnit, Easymock, Mockito, Coverage, Emma, Selenium e JMeter.</li>
					<li>Profilers: JProfile e VisualVM.</li>
				</ul>
			</li>
			<li>JavaScript: JQuery e Ext.JS.</li>
			<li>Ferramentas: Eclipse, Netbeans, IReport e Maven.</li>
			<li>Bancos de dados: Oracle, MS SQL, MySQL e PostgreSQL.</li>
			<li>Servidores: Tomcat, Glassfish, JBoss, Websphere e Weblogic.</li>
			<li>Controle de versão: CVS, SVN, Git e Mercurial.</li>
			<li>Mobile: Android.</li>
		</ul>
	</body>
</html>
